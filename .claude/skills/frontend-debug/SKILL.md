...
**1. Comprueba si ya está corriendo**, para no levantar una segunda instancia sobre un puerto ocupado. Si responde, ya está arriba: sáltate los pasos 2 a 5, dile al usuario contra qué puerto vas a trabajar y ve directo al paso 6.

```bash
curl -sS -o /dev/null -w "%{http_code}" http://localhost:<puerto>
```

**2. Pregunta al usuario qué entorno arrancar.** Lee los scripts de `package.json` — **no asumas que existe `dev` ni `start`** — y **no elijas el entorno por tu cuenta**, ni siquiera cuando uno parezca el obvio. La decisión es del usuario: pregúntasela con `AskUserQuestion` antes de ejecutar nada.

- Una opción por cada script que levante la app (`start:local`, `start:test`, `start:prod`, `dev:mock`…), con el nombre exacto del script como etiqueta.
- En la descripción de cada opción, lo que ese script implica de verdad: configuración (`--configuration=localhost`), puerto, y contra qué backend apunta si puedes deducirlo de los archivos de environment o de la config del framework. El usuario elige un entorno, no un string.
